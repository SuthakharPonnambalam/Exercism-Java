...
export function daysInBudget(budget, ratePerHour) {
  const workedDays = budget / (ratePerHour * 8);
  return Math.floor(workedDays);
}
...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const monthlyRate = billableDaysPerMonth * dayRate;
  const discountedMonthlyRate = monthlyRate * (1 - discount);
  
  const totalCost = (fullMonths * discountedMonthlyRate) + (remainingDays * dayRate);
